...
Depth as *leverage at the interface* — behaviour per unit of interface learned. Why the repo explicitly **rejects** Ousterhout's lines-of-implementation-to-lines-of-interface ratio ("rewards padding the implementation"). The deletion test. Java: `Ledger.post(Transfer)` versus a six-method shallow wrapper. AI parallel: the strongest one in the curriculum — a deep tool for an agent is a deep module, and for the same reason (interface = tokens the caller must hold in context).
