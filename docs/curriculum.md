...
**0002 — Deep vs shallow module**
Depth as *leverage at the interface* — behaviour per unit of interface learned. The repo rejects a lines-of-implementation-to-lines-of-interface ratio ("rewards padding the implementation") and attributes it to Ousterhout — **but the attribution is what fails.** He states no line-count ratio, and his own text falsifies one; the ratio he *does* state is `functionality/(interface complexity)`, neither term a line count. So `depth-as-leverage` restates him rather than correcting him, and the lesson teaches how a folk simplification gets attributed back to the author it distorts. Settled in `docs/research/05-ousterhout-depth.md`. The deletion test. Java: `Ledger.post(Transfer)` versus a six-method shallow wrapper. AI parallel: the strongest one in the curriculum — a deep tool for an agent is a deep module, and for the same reason (interface = tokens the caller must hold in context).
